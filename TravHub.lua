-- ╔══════════════════════════════════════════════════════════╗
-- ║  TravHub  ·  Project Delta  ·  v3.8  Crystal Edition     ║
-- ║  XENO FIX BUILD — Full error surfacing + HTTP compat     ║
-- ╚══════════════════════════════════════════════════════════╝
...
-- ══════════════════════════════════════════
--  FIX-XENO-01: VISIBLE ERROR SYSTEM
--  Shows errors in-game AND in executor console
--  so you can always see what's going wrong
-- ══════════════════════════════════════════
local function ShowError(msg)
    local short = tostring(msg):sub(1, 120)
    warn("[TravHub FATAL] " .. short)
    pcall(function()
...
local function ShowInfo(msg)
    warn("[TravHub] " .. tostring(msg))
end

-- Wrap the entire script in a top-level error catcher
-- so ANY crash gets surfaced to you instead of silent death
...
local function Safe(fn, ...)
    ...
    if not ok then ShowInfo("Safe(): " .. tostring(err)) end
end
...
-- ══════════════════════════════════════════
--  KEY AUTH CONFIG  —  fill in your values
-- ══════════════════════════════════════════
...
-- ══════════════════════════════════════════
--  FIX-XENO-02: HTTP REQUEST WRAPPER
--  Xeno uses the global `request()` function.
--  Original code checked syn.request first —
--  that doesn't exist on Xeno so detection
--  could silently fail on some builds.
--  New order: request → syn.request → http_request → http.request
-- ══════════════════════════════════════════
local function _kaRequest(opts)
    -- Detect HTTP function — Xeno uses global `request`
    local fn = nil
    if type(request) == "function" then
        fn = request                              -- Xeno, KRNL, Delta, Fluxus
    elseif syn and type(syn.request) == "function" then
        fn = syn.request                          -- Synapse X legacy
    elseif type(http_request) == "function" then
        fn = http_request                         -- Hydrogen, some mobile
    elseif http and type(http.request) == "function" then
        fn = http.request                         -- older API
    end
...
do
    local _savedKey = ""
    pcall(function()
        if type(isfile)=="function" and isfile("TravHub_v38_KAKey.txt") then
    ...
    -- FIX-XENO-03: Use PlayerGui directly — more reliable on all executors
    pcall(function()
        _gui.Parent = LocalPlayer.PlayerGui
    end)
    if not _gui.Parent then
        pcall(function()
            _gui.Parent = LocalPlayer:WaitForChild("PlayerGui", 5)
        end)
    end
    ...
    _ttl.Size = UDim2.new(1,0,0,48); _ttl.Position = UDim2.new(0,0,0,8)
    _ttl.BackgroundTransparency = 1; _ttl.Text = "TRAV HUB"
    _ttl.TextColor3 = Color3.fromRGB(200,130,255); _ttl.Font = Enum.Font.GothamBold
    _ttl.TextSize = 36; _ttl.ZIndex = 3; _ttl.Parent = _panel

    local _sub = Instance.new("TextLabel")
    _sub.Size = UDim2.new(1,0,0,20); _sub.Position = UDim2.new(0,0,0,54)
    _sub.BackgroundTransparency = 1; _sub.Text = "Project Delta  ·  Crystal Edition  ·  v3.8"
    _sub.TextColor3 = Color3.fromRGB(160,130,200); _sub.Font = Enum.Font.Gotham
    _sub.TextSize = 12; _sub.ZIndex = 3; _sub.Parent = _panel

    local _sepLine = Instance.new("Frame")
    _sepLine.Size = UDim2.new(1,-48,0,1); _sepLine.Position = UDim2.new(0,24,0,80)
    _sepLine.BackgroundColor3 = Color3.fromRGB(100,60,160); _sepLine.BorderSizePixel = 0
    _sepLine.ZIndex = 3; _sepLine.Parent = _panel

    local _step = Instance.new("TextLabel")
    _step.Size = UDim2.new(1,0,0,18); _step.Position = UDim2.new(0,0,0,88)
    _step.BackgroundTransparency = 1; _step.Text = "⟳  Connecting to KeyAuth..."
    _step.TextColor3 = Color3.fromRGB(160,130,200); _step.Font = Enum.Font.Gotham
    _step.TextSize = 12; _step.ZIndex = 3; _step.Parent = _panel

    local _prmpt = Instance.new("TextLabel")
    _prmpt.Size = UDim2.new(1,0,0,18); _prmpt.Position = UDim2.new(0,0,0,108)
    _prmpt.BackgroundTransparency = 1; _prmpt.Text = "Enter your license key:"
    _prmpt.TextColor3 = Color3.fromRGB(220,200,255); _prmpt.Font = Enum.Font.Gotham
    _prmpt.TextSize = 12; _prmpt.Visible = false; _prmpt.ZIndex = 3; _prmpt.Parent = _panel

    local _tb = Instance.new("TextBox")
    _tb.Size = UDim2.new(1,-80,0,34); _tb.Position = UDim2.new(0,40,0,130)
    _tb.BackgroundColor3 = Color3.fromRGB(28,8,58); _tb.BackgroundTransparency = 0.1
    _tb.BorderSizePixel = 0; _tb.TextColor3 = Color3.fromRGB(255,255,255)
    ...
    _tbStroke.Color = Color3.fromRGB(160,80,255); _tbStroke.Thickness = 1.5; _tbStroke.Parent = _tb

    local _hint = Instance.new("TextLabel")
    _hint.Size = UDim2.new(1,0,0,18); _hint.Position = UDim2.new(0,0,1,-26)
    _hint.BackgroundTransparency = 1; _hint.Text = "Please wait..."
    _hint.TextColor3 = Color3.fromRGB(120,100,160); _hint.Font = Enum.Font.Gotham
    _hint.TextSize = 11; _hint.ZIndex = 3; _hint.Parent = _panel

    -- Drag
    local _dragStrip = Instance.new("Frame")
    _dragStrip.Size = UDim2.new(1,0,0,128); _dragStrip.Position = UDim2.new(0,0,0,0)
    _dragStrip.BackgroundTransparency = 1; _dragStrip.BorderSizePixel = 0
    _dragStrip.ZIndex = 5; _dragStrip.Parent = _panel

    local _dragging,_dragStart,_panelStart = false,nil,nil
    _dragStrip.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.MouseButton1 then
            _dragging=true; _dragStart=inp.Position; _panelStart=_panel.Position
        end
    end)
    ...
    UserInputService.InputChanged:Connect(function(inp)
        if _dragging and inp.UserInputType==Enum.UserInputType.MouseMovement then
            ...
            _panel.Position=UDim2.new(_panelStart.X.Scale,_panelStart.X.Offset+d.X,_panelStart.Y.Scale,_panelStart.Y.Offset+d.Y)
        end
    end)
    ...
    local function _setHint(txt,col) _hint.Text=txt; _hint.TextColor3=col end
    local function _destroyAll() pcall(function() _gui:Destroy() end) end
    ...
    local function _validate(key)
        ...
        task.spawn(function()
            ...
            if ok then
                pcall(function() if type(writefile)=="function" then writefile("TravHub_v38_KAKey.txt",k) end end)
                ...
                _tb.Text=""
                _prmpt.Visible=true; _tb.Visible=true; _tb.TextEditable=true
                _busy=false
                task.wait(0.15)
    ...
    _tb.FocusLost:Connect(function(enterPressed)
        if enterPressed and not _busy then _validate(_tb.Text) end
    end)
    ...
    -- FIX-XENO-04: Timeout guard — if KeyAuth init takes >12s, skip and show input
    -- Prevents getting stuck forever if keyauth.win is slow to respond on Xeno
    ...
    task.spawn(function()
        ...
        if not initOk then
            ...
            _setHint("Retrying...",Color3.fromRGB(255,150,80))
            task.wait(3)
            initOk, initMsg = _kaInit()
            if not initOk then
                ShowInfo("KeyAuth retry failed: " .. initMsg .. " — showing input UI")
                -- FIX-XENO-05: On double failure, show input UI instead of just
                -- calling _fadeOut(). This lets users with valid keys still enter them.
                _step.Text = "⚠ Can't reach KeyAuth server"
                _step.TextColor3 = Color3.fromRGB(255,120,0)
        ...
        ShowInfo("KeyAuth connected. Checking saved key...")
        task.wait(0.4)
        _inputReady = true
...
ShowInfo("Key gate passed. Starting hub...")

-- ══════════════════════════════════════════
--  FRAME TICK  +  VIEWPORT CACHE
-- ══════════════════════════════════════════
local _frameTick = 0
...
-- ══════════════════════════════════════════
--  FIX-XENO-06: BOOT ANIMATION — wrapped in
--  hard pcall, never blocks hub load if it fails
-- ══════════════════════════════════════════
local function PlayBootAnimation()
    ...
    local byText     =NewDraw("Text", {Visible=false,Text="v3.8  ·  by Trav  ·  Xeno Fix Build",Size=12,Center=true,Outline=true,OutlineColor=Color3.new(0,0,0),Color=AME.white,Position=Vector2.new(cx,cy+16),Font=Drawing.Fonts.Gotham})
    ...
    for i=0,20 do
        ...
        panelBorder.Transparency=1-easeOut(t); panelInner.Transparency=1-easeOut(t)
        task.wait(0.013)
    end
...
-- ══════════════════════════════════════════
--  FIX-XENO-07: RAYFIELD LOADER
--  Primary URL + 2 fallback CDNs.
--  Shows a visible error if all fail.
-- ══════════════════════════════════════════
ShowInfo("Loading Rayfield UI library...")
local Rayfield = nil
local _rfURLs = {
    "https://sirius.menu/rayfield",
    "https://raw.githubusercontent.com/UI-Libraries/Rayfield/main/source.lua",
    "https://rawscripts.net/raw/Universal-UI-Library-Rayfield-12985",
}
for i, url in ipairs(_rfURLs) do
    ShowInfo("Trying Rayfield URL "..i..": "..url)
    local ok, result = pcall(function()
        return loadstring(game:HttpGet(url))()
    end)
    if ok and result then
        Rayfield = result
        ShowInfo("Rayfield loaded from URL "..i)
        break
    else
        ShowInfo("Rayfield URL "..i.." failed: "..tostring(result))
    end
end

if not Rayfield then
    ShowError("Rayfield failed to load from all URLs. Check executor HTTP & internet.")
    return
end
...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle="Crystal Edition  ·  Xeno Fix Build",
    Theme="Amethyst",
...
ShowInfo("Rayfield window created. Building UI...")
...
local State = {
    ...
    CustomCrosshair=false, CrosshairStyle="Plus",
    CrosshairColor=Color3.fromRGB(255,255,255),
    CrosshairSize=12, CrosshairGap=4, CrosshairThick=2,
    CrosshairRainbow=false, CrosshairSpin=false, CrosshairSpinSpeed=90,
    CrosshairOpacity=0,
    HitmarkerEnabled=false, GodModeEnabled=false,
    AntiRagdoll=false, AutoRespawn=false,
    ShowFPS=false, ShowPing=false,
}
...
local function KillAIESP(m)
    ...
    for _,k in ipairs({"boxOut","box","fill","name","dist","hpBg","hpBar","tracer"}) do
        pcall(function() o[k]:Remove() end)
    end
    AIESPObjects[m]=nil
end
KillAIESP_fn=KillAIESP
...
KillLootESP_fn=KillLootESP
local function HideLoot(o) o.label.Visible=false; o.dot.Visible=false; o.dist.Visible=false end

local _lootWasEnabled=false
local function UpdateLootESP(myRoot)
    if not State.LootESPEnabled then
        if _lootWasEnabled then for item in pairs(LootESPObjects) do KillLootESP(item) end; _lootWasEnabled=false end
        return
    end
    ...
    for inst,entry in pairs(lootCandidates) do
        ...
        local cat=entry.cat
        if not State.LootFilter[cat] then if LootESPObjects[inst] then KillLootESP(inst) end; continue end
        local pos
        if inst:IsA("BasePart") then pos=inst.Position
    ...
    for inst in pairs(LootESPObjects) do if not lootCandidates[inst] then KillLootESP(inst) end end
end
...
-- ══════════════════════════════════════════
local TRAP_KWS={
    mine="Mine",landmine="Mine",["land mine"]="Mine",antipersonnel="Mine",["ap mine"]="Mine",["pressure mine"]="Mine",
    claymore="Claymore",directional="Claymore",["m18"]="Claymore",["m18a1"]="Claymore",
    tripwire="Tripwire",["trip wire"]="Tripwire",["trip mine"]="Tripwire",tripmine="Tripwire",
    trap="Trap",explosive="Trap",ied="Trap",booby="Trap",proximity="Trap",sensor="Trap",
}
...
local function ClassifyTrap(name)
    ...
    for kw,trapType in pairs(TRAP_KWS) do
        if l:find(kw,1,true) then TrapNameCache[l]=trapType; return trapType end
    end
...
local function _TrapCheck(inst)
    ...
    local trapType=ClassifyTrap(n); if not trapType then return end
    local pos
    if inst:IsA("BasePart") then pos=inst.Position
    elseif inst:IsA("Model") then
        local p=inst.PrimaryPart or inst:FindFirstChildOfClass("BasePart")
...
local function _TrapRemove(inst)
    ...
    if TrapESPObjects[inst] then
        ...
        for _,k in ipairs({"dot","label","dist","ring","pulse","warn","dangerRing"}) do
            pcall(function() o[k]:Remove() end)
        end
...
local function HideTrap(o)
    ...
    o.warn.Visible=false; o.label.Visible=false; o.dist.Visible=false; o.dangerRing.Visible=false
end
local _trapWasEnabled=false
local function UpdateTrapESP(dt,myRoot)
    if not State.TrapESPEnabled then
        if _trapWasEnabled then for inst in pairs(TrapESPObjects) do _TrapRemove(inst) end; _trapWasEnabled=false end
        return
    end
    ...
    for inst,entry in pairs(trapCandidates) do
        ...
        if not pos then if TrapESPObjects[inst] then HideTrap(TrapESPObjects[inst]) end; continue end
        entry.pos=pos
        ...
        if dist>State.TrapMaxDist then if TrapESPObjects[inst] then HideTrap(TrapESPObjects[inst]) end; continue end
        local sp,onScreen=W2S(pos)
        local o=GetOrMakeTrapESP(inst,trapType)
        o.dot.Color=col; o.ring.Color=col; o.pulse.Color=col; o.label.Color=col; o.dangerRing.Color=col
        ...
        if State.TrapPulse then
            o.pulse.Position=sp; o.pulse.Radius=14+o._pulseT*22
            o.pulse.Transparency=pulseAlpha*0.2; o.pulse.Visible=true
        ...
        if State.TrapDangerRadius and myPos then
            ...
            if allOn then
                ...
                o.dangerRing.Position=sp; o.dangerRing.Radius=rScrn; o.dangerRing.Visible=true
            else o.dangerRing.Visible=false end
        else o.dangerRing.Visible=false end
    end
    for inst in pairs(TrapESPObjects) do if not trapCandidates[inst] then _TrapRemove(inst) end end
end

-- ══════════════════════════════════════════
--  EXFIL ESP
-- ══════════════════════════════════════════
local EXFIL_KWS={"exfil","extract","extraction","exit","evac","evacuate","escape","evacuation","chopper","helicopter","heli","gate","portal","hatch","depart","exit zone","extract zone","end zone"}
...
Workspace.DescendantAdded:Connect(function(d)
    task.defer(function()
        ...
        _LootCheck(d); _TrapCheck(d); _ExfilCheck(d)
    end)
end)
Workspace.DescendantRemoving:Connect(function(d)
    if d:IsA("Model") then aiCandidates[d]=nil end
    _LootRemove(d); _TrapRemove(d); _ExfilRemove(d)
end)
...
--  CROSSHAIR
-- ══════════════════════════════════════════
local _CH_cx,_CH_cy,_CH_ca,_CH_sa=0,0,1,0
...
local function _CHLine(d,ox1,oy1,ox2,oy2)
    ...
    d.Color=_CH_col; d.Thickness=_CH_th; d.Transparency=_CH_op; d.Visible=true
end
local function _CHLineOff(d,ox1,oy1,ox2,oy2,extraAngle)
    local a=chAngle+extraAngle; local ca,sa=mcos(a),msin(a); local cx,cy=_CH_cx,_CH_cy
    d.From=Vector2.new(cx+ox1*ca-oy1*sa,cy+ox1*sa+oy1*ca)
    d.To=Vector2.new(cx+ox2*ca-oy2*sa,cy+ox2*sa+oy2*ca)
    d.Color=_CH_col; d.Thickness=_CH_th; d.Transparency=_CH_op; d.Visible=true
end
...
local _smPts={false,false,false,false,false,false,false}
local function UpdateCrosshair(dt)
    if not State.CustomCrosshair then
        if _lastCHStyle~="" then HideCrosshair(); _lastCHStyle="" end; return
    end
    local style=State.CrosshairStyle
    if style~=_lastCHStyle then HideCrosshair(); _lastCHStyle=style; chAngle=0 end
    if State.CrosshairSpin then chAngle=(chAngle+mrad(State.CrosshairSpinSpeed)*dt)%(pi*2) end
    _CH_cx=VP_CX; _CH_cy=VP_CY; _CH_ca=mcos(chAngle); _CH_sa=msin(chAngle)
    _CH_col=State.CrosshairRainbow and RainbowHSV(0.6) or State.CrosshairColor
    _CH_th=State.CrosshairThick; _CH_op=State.CrosshairOpacity
    local cx,cy=_CH_cx,_CH_cy
    local col,th,op=_CH_col,_CH_th,_CH_op
    local gap,sz=State.CrosshairGap,State.CrosshairSize
    if style=="Plus" then
        _CHLine(CH.top,0,-gap,0,-(gap+sz)); _CHLine(CH.bot,0,gap,0,gap+sz)
        _CHLine(CH.left,-gap,0,-(gap+sz),0); _CHLine(CH.right,gap,0,gap+sz,0)
        CH.dot.Position=Vector2.new(cx,cy); CH.dot.Color=col; CH.dot.Transparency=op; CH.dot.Visible=true
    elseif style=="X" then
        local d45=pi/4
        _CHLineOff(CH.top,0,-gap,0,-(gap+sz),d45); _CHLineOff(CH.bot,0,gap,0,gap+sz,d45)
        _CHLineOff(CH.left,-gap,0,-(gap+sz),0,d45); _CHLineOff(CH.right,gap,0,gap+sz,0,d45)
        CH.dot.Position=Vector2.new(cx,cy); CH.dot.Color=col; CH.dot.Transparency=op; CH.dot.Visible=true
    ...
    elseif style=="T-Shape" then
        _CHLine(CH.top,0,-gap,0,-(gap+sz)); _CHLine(CH.left,-gap,0,-(gap+sz),0); _CHLine(CH.right,gap,0,gap+sz,0)
        CH.dot.Position=Vector2.new(cx,cy); CH.dot.Color=col; CH.dot.Transparency=op; CH.dot.Visible=true
    elseif style=="Happy Face" then
        local fR=sz+gap
        CH.eye1.Position=Vector2.new(cx-fR*0.35,cy-fR*0.3); CH.eye1.Radius=mmax(th,2); CH.eye1.Color=col; CH.eye1.Transparency=op; CH.eye1.Visible=true
        CH.eye2.Position=Vector2.new(cx+fR*0.35,cy-fR*0.3); CH.eye2.Radius=mmax(th,2); CH.eye2.Color=col; CH.eye2.Transparency=op; CH.eye2.Visible=true
        local smR=fR*0.52; local smCY=cy+fR*0.12
        local angStep=mrad(140)/6; local angStart=mrad(20)
        for i=0,6 do local ang=angStart+angStep*i; _smPts[i+1]=Vector2.new(cx+mcos(ang)*smR,smCY+msin(ang)*smR) end
        local sL={CH.sm1,CH.sm2,CH.sm3,CH.sm4,CH.sm5,CH.sm6}
        for i=1,6 do local d=sL[i]; d.From=_smPts[i]; d.To=_smPts[i+1]; d.Color=col; d.Thickness=th; d.Transparency=op; d.Visible=true end
    elseif style=="KovaaK" then
        _CHLine(CH.top,0,-gap,0,-(gap+sz)); _CHLine(CH.bot,0,gap,0,gap+sz)
        _CHLine(CH.left,-gap,0,-(gap+sz),0); _CHLine(CH.right,gap,0,gap+sz,0)
        local tOff=gap+sz+4; local tLen=5
        _CHLine(CH.sn1,0,-(tOff),0,-(tOff+tLen)); _CHLine(CH.sn2,0,tOff,0,tOff+tLen)
        _CHLine(CH.sn3,-(tOff),0,-(tOff+tLen),0); _CHLine(CH.sn4,tOff,0,tOff+tLen,0)
        CH.dot.Position=Vector2.new(cx,cy); CH.dot.Color=col; CH.dot.Transparency=op; CH.dot.Visible=true
    elseif style=="Sniper" then
        ...
        for _,k in ipairs({"sn1","sn2","sn3","sn4"}) do CH[k].Color=col; CH[k].Thickness=1; CH[k].Transparency=snOp; CH[k].Visible=true end
        CH.ring.Position=Vector2.new(cx,cy); CH.ring.Color=col; CH.ring.Radius=mmax(gap,6); CH.ring.Thickness=1; CH.ring.Transparency=op; CH.ring.Visible=true
    ...
    elseif style=="Diamond" then
        local r=sz+gap
        local dtop=_CHPt(0,-r); local drgt=_CHPt(r,0); local dbot=_CHPt(0,r); local dlft=_CHPt(-r,0)
    ...
    elseif style=="Bracket" then
        ...
        local x1=cx-gap-bSz; local x2=cx+gap+bSz; local y1=cy-gap-bSz; local y2=cy+gap+bSz
...
local function UpdateHitmarker(dt)
    ...
    hmTimer=hmTimer-dt
    if hmTimer<=0 then
        hmActive=false; HM.tl.Visible=false; HM.tr.Visible=false; HM.bl.Visible=false; HM.br.Visible=false; return
    end
    local cx=VP_CX; local cy=VP_CY; local s=9; local fade=mclamp(hmTimer/0.28,0,1); local tr=1-fade
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if delta>0.0004 then
        ...
        pcall(function()
            Camera.CFrame=CFrame.new(camCF.Position)*CFrame.fromEulerAnglesYXZ(ccx-delta*(1-State.RecoilStrength)+_nrJitter,ccy,ccz)
        end)
    end
...
local _trigAcc=0
RunService.Heartbeat:Connect(function(dt)
    if not State.TriggerBot or not VIM then _trigAcc=0; return end
    local target=Mouse.Target
    if not target then _trigAcc=0; return end
    local model=target:FindFirstAncestorOfClass("Model")
    if not model then _trigAcc=0; return end
    local p=Players:GetPlayerFromCharacter(model)
    if not p or p==LocalPlayer or IsAimTeammate(p) then _trigAcc=0; return end
    local char=p.Character; if not char then _trigAcc=0; return end
    local hum=char:FindFirstChildOfClass("Humanoid")
    if not hum or hum.Health<=0 then _trigAcc=0; return end
    local lmb=UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1)
    if lmb then _trigAcc=0; return end
    _trigAcc=_trigAcc+dt
    local needed=(State.TrigDelay or 0)/1000
        ...
        for _,p in ipairs(Players:GetPlayers()) do
            ...
            if root then
                if not HitboxObjects[p] or not HitboxObjects[p].Parent then
                    ...
                    part.CastShadow=false; part.Size=Vector3.new(State.HitboxSize,State.HitboxSize,State.HitboxSize)
                    part.CFrame=root.CFrame; part.Parent=char
                    HitboxObjects[p]=part
                else
                    local part=HitboxObjects[p]
                    part.CFrame=root.CFrame
                    if part.Size.X~=State.HitboxSize then part.Size=Vector3.new(State.HitboxSize,State.HitboxSize,State.HitboxSize) end
                end
            else
...
RunService.RenderStepped:Connect(function(dt)
    ...
    FOVCircle.Radius=State.AimbotFOV; FOVCircle.Color=State.FOVColor
    FOVCircle.Visible=State.AimbotEnabled and State.FOVVisible
    RunAimbot(camCF); RunAIAimbot(camCF)
    ...
    UpdateCrosshair(dt)
    UpdateHitmarker(dt)
    if State.ShowFPS then FPSLabel.Text=sfmt("FPS: %d",fpsVal); FPSLabel.Visible=true else FPSLabel.Visible=false end
    if State.ShowPing then PingLabel.Text=sfmt("Ping: %d ms",pingVal); PingLabel.Visible=true else PingLabel.Visible=false end
end)
...
T1:CreateToggle({Name="Enable Player ESP",CurrentValue=false,Flag="E_ESP",Callback=function(v) State.ESPEnabled=v; if not v then for _,o in pairs(ESPObjects) do HideESP(o) end end end})
...
T1:CreateToggle({Name="Corner Box Style",CurrentValue=false,Flag="E_CRN",Callback=function(v)State.ESPCornerBox=v end})
...
T2:CreateToggle({Name="Enable AI ESP",CurrentValue=false,Flag="AI_ON",Callback=function(v) State.AIESPEnabled=v; if not v then for m,_ in pairs(AIESPObjects) do KillAIESP(m) end end end})
...
T2:CreateToggle({Name="Enable AI Aimbot",CurrentValue=false,Flag="AA_ON",Callback=function(v) State.AIAimbotEnabled=v; AIFOVCircle.Visible=v and State.AIFOVVisible; if not v then AILockDot.Visible=false; aiLockedTarget=nil end end})
T2:CreateDropdown({Name="Aim Mode",Options={"Smooth","Blatant","Instant"},CurrentOption={"Smooth"},Flag="AA_MODE",Callback=function(v)State.AIAimbotMode=v[1] or "Smooth" end})
T2:CreateDropdown({Name="Aim Key",Options={"MouseButton2","C","Q","E","F"},CurrentOption={"MouseButton2"},Flag="AA_KEY",Callback=function(v)State.AIAimbotKey=v[1] or "MouseButton2" end})
T2:CreateDropdown({Name="Target Part",Options={"Head","HumanoidRootPart","UpperTorso","Torso"},CurrentOption={"Head"},Flag="AA_PART",Callback=function(v)State.AIAimbotPart=v[1] or "Head"; aiLockedTarget=nil end})
T2:CreateToggle({Name="Lock-On",CurrentValue=false,Flag="AA_LOCK",Callback=function(v)State.AIAimbotLock=v; aiLockedTarget=nil end})
T2:CreateSlider({Name="Smooth Speed",Range={1,25},Increment=1,CurrentValue=5,Flag="AA_SS",Callback=function(v)State.AIAimbotSmooth=0.025+v*0.014 end})
T2:CreateSlider({Name="Snap Speed",Range={30,99},Increment=1,Suffix="%",CurrentValue=55,Flag="AA_BS",Callback=function(v)State.AIAimbotBlatant=v/100 end})
T2:CreateSlider({Name="FOV Radius",Range={20,700},Increment=5,Suffix=" px",CurrentValue=150,Flag="AA_FOVR",Callback=function(v)State.AIAimbotFOV=v end})
T2:CreateToggle({Name="Show FOV Circle",CurrentValue=true,Flag="AA_FOVV",Callback=function(v)State.AIFOVVisible=v; AIFOVCircle.Visible=State.AIAimbotEnabled and v end})
T2:CreateColorPicker({Name="FOV Color",Color=Color3.fromRGB(255,160,30),Flag="AA_FOVC",Callback=function(v)State.AIFOVColor=v; AIFOVCircle.Color=v end})

local T3=Window:CreateTab("💎  Loot ESP",4483362458)
T3:CreateSection("Master")
T3:CreateToggle({Name="Enable Loot ESP",CurrentValue=false,Flag="L_ON",Callback=function(v) State.LootESPEnabled=v; if not v then for i,_ in pairs(LootESPObjects) do KillLootESP(i) end end end})
...
T3:CreateToggle({Name="🔑 Keys",     CurrentValue=true, Flag="LF_K", Callback=function(v)State.LootFilter.Keys=v end})
T3:CreateToggle({Name="💀 Bodies",   CurrentValue=true, Flag="LF_B", Callback=function(v)State.LootFilter.Bodies=v end})
...
T3:CreateToggle({Name="🛡 Armor",    CurrentValue=true, Flag="LF_AR",Callback=function(v)State.LootFilter.Armor=v end})
T3:CreateToggle({Name="💣 Explosives",CurrentValue=true,Flag="LF_EX",Callback=function(v)State.LootFilter.Explosives=v end})
T3:CreateToggle({Name="🔧 Tools",    CurrentValue=true, Flag="LF_TL",Callback=function(v)State.LootFilter.Tools=v end})
...
T4:CreateToggle({Name="Enable Exfil ESP",CurrentValue=false,Flag="X_ON",Callback=function(v) State.ExfilESPEnabled=v; if not v then for i,_ in pairs(ExfilESPObjects) do KillExfilESP(i) end; ExfilArrow.shaft.Visible=false; ExfilArrow.head1.Visible=false; ExfilArrow.head2.Visible=false; ExfilArrow.label.Visible=false end end})
...
T4B:CreateToggle({Name="Enable Trap ESP",CurrentValue=false,Flag="TR_ON",Callback=function(v) State.TrapESPEnabled=v; if not v then for inst in pairs(TrapESPObjects) do _TrapRemove(inst) end end end})
...
T4B:CreateToggle({Name="Danger Radius",CurrentValue=true, Flag="TR_DR", Callback=function(v)State.TrapDangerRadius=v end})
T4B:CreateSlider({Name="Radius Size",Range={2,20},Increment=1,Suffix=" studs",CurrentValue=8,Flag="TR_DRS",Callback=function(v)State.TrapDangerRadiusSize=v end})
T4B:CreateSlider({Name="Max Distance",Range={25,500},Increment=25,Suffix=" studs",CurrentValue=150,Flag="TR_MD",Callback=function(v)State.TrapMaxDist=v end})
...
T5:CreateToggle({Name="Enable Aimbot",CurrentValue=false,Flag="A_ON",Callback=function(v) State.AimbotEnabled=v; FOVCircle.Visible=v and State.FOVVisible; if not v then LockDot.Visible=false; lockedTarget=nil end end})
T5:CreateDropdown({Name="Aim Mode",Options={"Smooth","Blatant","Instant"},CurrentOption={"Smooth"},Flag="A_MODE",Callback=function(v)State.AimbotMode=v[1] or "Smooth" end})
T5:CreateDropdown({Name="Aim Key",Options={"MouseButton2","C","Q","E","F"},CurrentOption={"MouseButton2"},Flag="A_KEY",Callback=function(v)State.AimKey=v[1] or "MouseButton2" end})
...
T5:CreateDropdown({Name="Target Part",Options={"Head","HumanoidRootPart","UpperTorso","Torso","LeftUpperArm","RightUpperArm"},CurrentOption={"Head"},Flag="A_PART",Callback=function(v)State.AimbotPart=v[1] or "Head"; lockedTarget=nil end})
T5:CreateSection("Smooth")
T5:CreateSlider({Name="Smooth Speed",Range={1,25},Increment=1,CurrentValue=5,Flag="A_SS",Callback=function(v)State.AimbotSmooth=0.025+v*0.014 end})
T5:CreateSection("Blatant")
T5:CreateSlider({Name="Snap Speed",Range={30,99},Increment=1,Suffix="%",CurrentValue=55,Flag="A_BS",Callback=function(v)State.AimbotBlatant=v/100 end})
...
T5:CreateSlider({Name="Strength",Range={1,10},Increment=1,CurrentValue=5,Flag="A_PRS",Callback=function(v)State.AimbotPredictStr=v/10 end})
T5:CreateSection("FOV Circle")
T5:CreateSlider({Name="FOV Radius",Range={20,700},Increment=5,Suffix=" px",CurrentValue=150,Flag="A_FOVR",Callback=function(v)State.AimbotFOV=v end})
T5:CreateToggle({Name="Show FOV Circle",CurrentValue=true,Flag="A_FOVV",Callback=function(v)State.FOVVisible=v; FOVCircle.Visible=State.AimbotEnabled and v end})
T5:CreateColorPicker({Name="FOV Color",Color=Color3.fromRGB(180,100,255),Flag="A_FOVC",Callback=function(v)State.FOVColor=v; FOVCircle.Color=v end})

local T6=Window:CreateTab("🔭  Zoom",4483362458)
T6:CreateSection("Scope Zoom")
T6:CreateToggle({Name="Enable Zoom",CurrentValue=false,Flag="Z_ON",Callback=function(v) State.ZoomEnabled=v; if not v and State.ZoomActive then State.ZoomActive=false; CancelZoomTween(); Camera.FieldOfView=State.CameraFOV; ShowZoomOverlay(false) end end})
T6:CreateDropdown({Name="Zoom Key",Options={"Z","X","V","F","G"},CurrentOption={"Z"},Flag="Z_KEY",Callback=function(v)State.ZoomKey=v[1] or "Z" end})
T6:CreateSlider({Name="Zoom FOV",Range={5,60},Increment=1,Suffix="°",CurrentValue=20,Flag="Z_FOV",Callback=function(v)State.ZoomFOV=v; if State.ZoomActive then SetCameraFOV(v,State.ZoomSmooth,0.10) end end})
T6:CreateSlider({Name="Zoom Speed",Range={5,30},Increment=1,CurrentValue=18,Flag="Z_SPD",Callback=function(v)State.ZoomSpeed=v/100 end})
T6:CreateToggle({Name="Smooth Transition",CurrentValue=true,Flag="Z_SM",Callback=function(v)State.ZoomSmooth=v end})
T6:CreateToggle({Name="Scope Overlay",CurrentValue=true,Flag="Z_OVL",Callback=function(v)State.ZoomOverlay=v; if not v then ShowZoomOverlay(false) end end})
T6:CreateColorPicker({Name="Overlay Color",Color=Color3.fromRGB(180,220,255),Flag="Z_OVC",Callback=function(v)State.ZoomOverlayColor=v end})

local T7=Window:CreateTab("⚔️  Combat",4483362458)
T7:CreateSection("Recoil")
T7:CreateToggle({Name="No Recoil",CurrentValue=false,Flag="C_NR",Callback=function(v)State.NoRecoilEnabled=v; lastCamCF=nil end})
T7:CreateSlider({Name="Reduction %",Range={10,100},Increment=5,Suffix="%",CurrentValue=80,Flag="C_NRS",Callback=function(v)State.RecoilStrength=1-(v/100) end})
T7:CreateSection("Hitbox")
T7:CreateToggle({Name="Hitbox Expander",CurrentValue=false,Flag="C_HBX",Callback=function(v)State.HitboxExpander=v; StartHitboxExpander() end})
T7:CreateSlider({Name="Hitbox Size",Range={2,20},Increment=1,Suffix=" studs",CurrentValue=6,Flag="C_HBXS",Callback=function(v)State.HitboxSize=v end})
...
T8:CreateSlider({Name="Field of View",Range={50,120},Increment=1,Suffix="°",CurrentValue=70,Flag="V_FOV",Callback=function(v) State.CameraFOV=v; if not State.ZoomActive then CancelZoomTween(); Camera.FieldOfView=v end end})
T8:CreateSection("Crosshair")
T8:CreateToggle({Name="Enable Crosshair",CurrentValue=false,Flag="V_CH",Callback=function(v)State.CustomCrosshair=v; if not v then HideCrosshair(); _lastCHStyle="" end end})
T8:CreateDropdown({Name="Style",Options={"Plus","X","Dot","Circle","T-Shape","Happy Face","KovaaK","Sniper","Diamond","Bracket"},CurrentOption={"Plus"},Flag="V_CHS",Callback=function(v)State.CrosshairStyle=v[1] or "Plus" end})
T8:CreateToggle({Name="🌈 Rainbow", CurrentValue=false,Flag="V_CHRBW",Callback=function(v)State.CrosshairRainbow=v end})
T8:CreateToggle({Name="🔄 Spinning",CurrentValue=false,Flag="V_CHSP", Callback=function(v)State.CrosshairSpin=v; if not v then chAngle=0 end end})
...
T9:CreateToggle({Name="God Mode",   CurrentValue=false,Flag="SV_GOD",Callback=function(v) State.GodModeEnabled=v; SetupGodMode(LocalPlayer.Character); if not v and godConn then godConn:Disconnect(); godConn=nil end end})
T9:CreateToggle({Name="Anti-Ragdoll",CurrentValue=false,Flag="SV_RAG",Callback=function(v) State.AntiRagdoll=v; if v then ApplyAntiRagdoll(LocalPlayer.Character) end end})
...
T10:CreateButton({Name="Rejoin",Callback=function() Notify("TravHub","Rejoining...",2); task.wait(0.5); TeleportService:Teleport(game.PlaceId,LocalPlayer) end})
T10:CreateButton({Name="Server Hop",Callback=function()
...
T10:CreateButton({Name="Kill Particles",Callback=function()
...
T10:CreateLabel("TravHub v3.8  ·  Crystal Edition  ·  Xeno Fix Build")
T10:CreateLabel("FIX-01 HTTP order  ·  FIX-02 Rayfield fallbacks")
T10:CreateLabel("FIX-03 PlayerGui direct  ·  FIX-04 KeyAuth timeout")
T10:CreateLabel("FIX-05 Silent error surfacing  ·  Stay lowkey 🤫")

-- ══════════════════════════════════════════
--  BOOTSTRAP + STARTUP
-- ══════════════════════════════════════════
task.defer(function()
    _AIBootstrap(); _LootBootstrap(); _TrapBootstrap(); _ExfilBootstrap()
end)
Rayfield:LoadConfiguration()
task.wait(0.3)
Notify("TravHub v3.8 ✦","Xeno Fix Build loaded — check executor console if any issues ✨",6)
ShowInfo("Hub fully loaded. All systems active.")

-- ══════════════════════════════════════════
--  END OF TOP-LEVEL XPCALL WRAPPER
-- ══════════════════════════════════════════
end, function(err)
...
if not _scriptOk then
    -- Surface the full error with traceback to both console and in-game
    warn("[TravHub FATAL CRASH]\n" .. tostring(_scriptErr))
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title    = "⚠ TravHub Crashed",
            Text     = tostring(_scriptErr):sub(1, 140),
            Duration = 15,
        })
    end)
end
